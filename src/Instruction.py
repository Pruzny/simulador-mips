...
class Steps:
    ...
    def skip(self, instructions: list[Instruction], index: int):
        if 0 <= index - 4 < len(instructions):
            self.WB = instructions[index]
        if 0 <= index - 3 < len(instructions):
            self.MEM = instructions[index]
        if 0 <= index - 2 < len(instructions):
            self.EX = instructions[index]
        if 0 <= index - 1 < len(instructions):
            self.ID = instructions[index]
        if 0 <= index < len(instructions):
            self.IF = instructions[index]


class InstructionSteps:

    def __init__(self, instruction: Instruction):
        self.instruction_base = instruction
        self.ID = None
        self.IF = None
        self.EX = None
        self.MEM = None
    ...
    def setEtapa(self, stretapa: str):
        self.IF = False
        self.ID = False
        self.EX = False
        self.MEM = False
        self.WB = False
        if stretapa.upper() == "ID":
            self.ID = True
        elif stretapa.upper() == "IF":
            self.IF = True
        elif stretapa.upper() == "EX":
            self.EX = True
        elif stretapa.upper() == "MEM":
            self.MEM = True
        elif stretapa.upper() == "WB":
            self.WB = True
